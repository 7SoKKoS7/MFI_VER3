...
```
- Strong по умолчанию требует: 2 закрытия H1 за pivot в сторону входа И ретест уровня на M15 (по желанию также M5) с отскоком (тенью) и (опционально) объёмом.
...
- **Использование завершенных баров** для анализа объема
 - **MF‑pivot подтверждается** только при выполнении двух условий: цена прошла `InpDeviation` и прошло минимум `InpDepth` баров с момента последнего кандидата на экстремум.
  ...
  - Cold-start: первые секунды ZigZag не дергается (M5/M15/H1 — 2s, H4 — 6s, D1 — 10s). UI сразу показывает кэш/fast-pivot и позже бесшовно обновляется ZigZag-значениями.
  - Мемоизация: пивоты пересчитываются только на новом закрытом баре TF; между барами возвращается кэш (экономит CopyBuffer).
...
- При старте/переключении UI сначала показывает кэш, а затем обновляет значения онлайн, когда ZigZag готов.
 ...
 - UI рендерится сразу в OnInit из кэша (GV), без ожидания таймера/тика.
 - Тренды (H1/M15/M5) сохраняются в Global Variables и подхватываются всеми инстансами, поэтому направление H1 будет одинаковым независимо от текущего графика.
